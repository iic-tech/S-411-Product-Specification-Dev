...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ice="http://www.iho.int/ice"
    ...
    <xsl:template name="lkilneTemplate" match="ice:lkilne">
        <xsl:call-template name="lineSimpleStyleTemplate">
            <xsl:with-param name="featureReference" select="ice:lkilne/@gml:id"/>
            ...
            <xsl:with-param name="colorToken" select="'0 255 0'"/>

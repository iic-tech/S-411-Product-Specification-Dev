...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template name="icethkTemplate" match="ice:icethk">
        <xsl:variable name="icetty" select="ice:icethk/ice:icetty"/>
        <xsl:variable name="icetck" select="ice:icethk/ice:icetck"/>
        <xsl:variable name="id" select="ice:icethk/@gml:id"/>
        ...
        <xsl:choose>
            <xsl:when test="number($icetty) = 1">
                <xsl:call-template name="pointSimpleSymbolTemplate">
                    ...
                    <xsl:with-param name="symbolReference" select="'../../Symbols/icethk_measured.svg'"/>
                    <xsl:with-param name="rotation" select="'0.0'"/>
                    <xsl:with-param name="scaleFactor" select="'1.0'"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="number($icetty) = 2">
                <xsl:call-template name="pointSimpleSymbolTemplate">
                    <xsl:with-param name="featureReference" select="$id"/>
                    <xsl:with-param name="viewingGroup" select="$viewingGroup"/>
                    <xsl:with-param name="displayPlane" select="$displayPlane"/>
                    <xsl:with-param name="drawingPriority" select="$drawingPriority"/>
                    <xsl:with-param name="symbolReference" select="'../../Symbols/icethk_estimated.svg'"/>
                    <xsl:with-param name="rotation" select="'0.0'"/>
                    <xsl:with-param name="scaleFactor" select="'1.0'"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="number($icetty) = 99">
                <xsl:call-template name="pointSimpleSymbolTemplate">
                    <xsl:with-param name="featureReference" select="$id"/>
                    <xsl:with-param name="viewingGroup" select="$viewingGroup"/>
                    <xsl:with-param name="displayPlane" select="$displayPlane"/>
                    <xsl:with-param name="drawingPriority" select="$drawingPriority"/>
                    <xsl:with-param name="symbolReference" select="'../../Symbols/icethk_unknown.svg'"/>
                    <xsl:with-param name="rotation" select="'0.0'"/>
                    <xsl:with-param name="scaleFactor" select="'1.0'"/>
                </xsl:call-template>
            </xsl:when>
        ...
        <xsl:call-template name="pointSimpleLabelTemplate">
            ...
            <xsl:with-param name="displayPlane" select="$displayPlane"/>
            <xsl:with-param name="drawingPriority" select="$drawingPriority"/>
            <xsl:with-param name="text" select="$icetck"/>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template name="snwcvrTemplate" match="ice:snwcvr">
        <xsl:variable name="icescn" select="ice:snwcvr/ice:icescn"/>
        <xsl:variable name="icesct" select="ice:snwcvr/ice:icesct"/>
        <xsl:variable name="id" select="ice:snwcvr/@gml:id"/>
        ...
        <xsl:call-template name="pointSimpleSymbolTemplate">
            ...
            <xsl:with-param name="symbolReference" select="'../../Symbols/snwcvr.svg'"/>
        ...
        <xsl:call-template name="pointSimpleLabelTemplate">
            ...
            <xsl:with-param name="displayPlane" select="$displayPlane"/>
            <xsl:with-param name="drawingPriority" select="$drawingPriority"/>
            <xsl:with-param name="text" select="$icescn"/>
        ...
        <xsl:call-template name="pointSimpleLabelTemplate">
            ...
            <xsl:with-param name="displayPlane" select="$displayPlane"/>
            <xsl:with-param name="drawingPriority" select="$drawingPriority"/>
            <xsl:with-param name="text" select="$icesct"/>
            <xsl:with-param name="horizontalAlignment" select="'Center'"/>
            <xsl:with-param name="verticalAlignment" select="'Center'"/>
            <xsl:with-param name="bodySize" select="'3.0'"/>
            <!--<xsl:with-param name="verticalOffset"/>-->
            ...
            <xsl:with-param name="fontProportion"/>
            <xsl:with-param name="rotation"/>-->
            <xsl:with-param name="offset_X" select="'2.0'"/>
            <xsl:with-param name="offset_Y" select="'0.0'"/>
        </xsl:call-template>
    </xsl:template>
</xsl:stylesheet>